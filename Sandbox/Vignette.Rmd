...
SF_hh_seed <- SF_pums %>% 
  ...
  summarise(across(.cols = everything(),
                   .fns = first)) %>% 
  # Join to get tract geography
  left_join(cts_to_pumas, by = c("PUMA" = "PUMA5CE")) %>% 
  # Cleanup
  dplyr::select(SERIALNO, CTFP, HHINC_CAT, HHT, NP, WGTP) %>% 
  rename("geo_tract" = CTFP,
         "hhincome" = HHINC_CAT,
         "hhsize" = NP,
         "hhtype" = HHT, 
         "weight" = WGTP)
...
wrk_totals <- acs_occup_tgt %>% group_by(geo_tract) %>% summarise(estimate = sum(estimate)) #Total working population
...
sum(acs_occup_tgt %>% group_by(geo_tract) %>% summarise(tpop = sum(estimate)) %>% left_join(pop_totals) %>% mutate(errors = tpop-estimate) %>% pull(errors)) == 0
