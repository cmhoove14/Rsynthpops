...
The next step is to generate the household and person seed dataframes. These "seeds" contain target proportions for characteristics of the target population that will be used in iterative proportional fitting along with target population characteristics to generate the population. To generate seeds, we need to incorporate our desired household and person characteristics and the survey weights to estimate. We'll start with the household seed data, and the first step is to determine which of our chosen variables are at the household level
...
vt_hh_seed <- vt_pums %>% 
  ...
  # Get totals in each category
  count(PUMA, NP, HHINC_CAT, wt = WGTP, name = "n_group") %>% 
  # Join with overall toals to get proportions
  left_join(., pumas_count, by = "PUMA") %>% 
  mutate(PROP = n_group/n_tot)
  
# Verify that props within PUMAs sum to 1
vt_hh_seed %>% group_by(PUMA) %>% summarise(prop_sum = sum(PROP)) %>% pull(prop_sum)
```




For now, we won't include anyone living in group quarters, so we'll remove anyone with the "GQ" tag in their serial number
