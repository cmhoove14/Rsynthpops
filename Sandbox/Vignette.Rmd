...
SF_hh_seed <- SF_pums %>% 
  ...
  # Cleanup
  dplyr::select(SERIALNO, HHINC_CAT, HHT, NP, WGTP) %>% 
...
SF_p_seed <- SF_pums %>% 
  ...
  left_join(acs_occp_lookup %>% dplyr::select(Code, d2_code),
            by = c("occupation" = "Code")) %>% 
  mutate(d2_code = if_else(occupation == "0009", 9, d2_code))
...
**TO-DO: Start compiling reference of PUMS to ACS variable matches**
...
```{r h_targets}
# household size -----------------------
acs_hhsize_tgt <- acs_hhsize %>%
  filter(county_fips == fips_use) %>% 
  mutate(hhtype = if_else(grepl("fam", variable), 1, 2),
         hhsize = parse_number(variable)) %>% 
  dplyr::select(GEOID, estimate, hhtype, hhsize) %>% 
  pivot_wider(names_from = hhsize,values_from = estimate)

# household income ---------------
acs_hh_income_tgt <- acs_hh_income %>%
  filter(county_fips == fips_use) %>% 
  ...
  mutate(hhincome = as.factor(case_when(income_max < 50000 ~ 1,
                                        ...
                                        income_min >= 100000 ~3))) %>% 
  dplyr::select(GEOID, estimate, hhincome)

```

```{r p_targets}
# Age by sex -------------
acs_age_sex_tgt <- acs_age_sex_state %>%
  filter(county_fips == fips_use) %>% 
  ...
  mutate(age_cat = case_when(age_max < 10 ~ 1,
                             ...
                             age_min >= 80 ~9)) %>% 
  dplyr::select(GEOID, estimate, sex_num, age_cat) %>% 
  rename("sex" = sex_num) %>% 
  group_by(GEOID,sex,age_cat) %>% 
  summarise(estimate = sum(estimate)) %>% 
  ungroup() %>% 
  pivot_wider(names_from = )

# Race ---------------------
acs_race_tgt <- acs_race_state %>%
  filter(county_fips == fips_use) %>% 
  ...
  dplyr::select(GEOID, estimate, hispanic, race) %>% 
  group_by(GEOID,hispanic,race) %>% 
  summarise(estimate = sum(estimate)) %>% 
  ungroup()
...
pop_totals <- acs_age_sex_tgt %>% group_by(GEOID) %>% summarise(estimate = sum(estimate)) # Total population
wrk_totals <- acs_occup_tgt %>% group_by(GEOID) %>% summarise(estimate = sum(estimate)) #Total working population

nwrk_pop <- pop_totals %>% left_join(wrk_totals, by = "GEOID") %>% #Total non-working population
  mutate(d2_code = 9,
         estimate = estimate.x - estimate.y) %>% 
  dplyr::select(GEOID, d2_code, estimate)

acs_occup_tgt <- rbind(acs_occup_tgt, nwrk_pop)

#Check to make sure population classified into working groups is equal to total population
sum(acs_occup_tgt %>% group_by(GEOID) %>% summarise(tpop = sum(estimate)) %>% left_join(pop_totals) %>% mutate(errors = tpop-estimate) %>% pull(errors)) == 0
